...
const HeroSection = () => {
    return (
        <div className='w-full mt-32 md:mt-36 xl:mt-40 relative'>
            <h1 className="text-[50px] leading-14 text-center text-white/90 md:text-left sm:text-[55px] md:text-[64px] xl:text-[84px] tracking-wide font-bold w-full">FRONTEND DEVELOPER</h1>
            <div className="flex flex-col lg:flex-row items-center justify-start gap-6 xl:gap-16">
                <div className="w-[260px] h-[260px] sm:w-[305px] sm:h-[305px] rounded-full overflow-hidden mt-10 relative">
                    <div className="absolute inset-0 rounded-full overflow-hidden border-3 border-dashed border-gray-400/80
                     animate-spin [animation-duration:30s] z-20" />
                ...
                <div className="w-full max-w-96 md:max-w-[500px] lg:w-[58%] xl:w-1/2 mt-5 xl:mt-24 flex flex-col items-start gap-6">
                    ...
                    <div className="w-full flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 md:gap-14 lg:gap-10 text-sm sm:text-base xl:text-lg font-semibold">
                        <button className=" sm:w-auto flex cursor-pointer bg-white/80 text-gray-900/90 rounded-xl px-3 xl:px-6 items-center gap-2 h-12"><span>Let's Connect</span><FaRegHandshake size={25} className="inline" /></button>
                        <button className=" sm:w-auto text-white/80 flex cursor-pointer ring-1 ring-gray-500/50 rounded-xl px-3 xl:px-6 gap-2 items-center h-12"><span>Explore my works</span> <GoArrowDown size={25} className="inline" /></button>
                    </div>
                </div>
            </div>
            {/* Top glow */}
